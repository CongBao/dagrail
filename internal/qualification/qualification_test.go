...
func TestSourceQualificationIsStructuralAndExplicitlyNotProductionValidation(t *testing.T) {
	...
	if !report.StructuralCandidate || report.ProductionValidated || report.ProjectEvidence || len(report.Checks) != 10 || len(report.ExternalGates) != 8 || len(report.AdoptionGaps) != 4 {
