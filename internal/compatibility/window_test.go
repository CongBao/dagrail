...
func TestEmbeddedBetaWindowIsClosedAndSchemaValid(t *testing.T) {
	...
	if window.CurrentVersion != version.Version || evidence.Historical != 27 || !strings.HasPrefix(evidence.Digest, "sha256:") {
...
func TestBetaWindowRejectsMutationAndOmission(t *testing.T) {
	...
	for name, mutation := range map[string]string{
		"wrong candidate": strings.Replace(string(raw), `"currentVersion": "0.26.5"`, `"currentVersion": "1.0.0"`, 1),
...
func TestHistoricalWindowDocumentationMatchesClosedManifest(t *testing.T) {
	const exactWindow = "v0.10.0–v0.26.4"
